describe('Kitchen Sink Participation', () => {
  ...
  before(function () {
    cy.createConvo('moderator').then(() => {
      ...
      cy.get('input[data-test-id="strict_moderation"]')
        .check()
        .should('be.checked')
      cy.get('input[data-test-id="auth_needed_to_write"]')
        .uncheck()
        .should('not.be.checked')
      cy.get('input[data-test-id="auth_needed_to_vote"]')
        .uncheck()
        .should('not.be.checked')
      ...
      const preVisualizationVotes = ['AAD', 'AAD', 'AAA', 'DDD', 'DAD']
